...
class DataModelsCase(unittest.TestCase):
    def test_happy(self):
        ...
        task_statuses_list = []
        for n, c in [('Todo', '#f5f5f5'), ('Work In Progress', '#3273dc'), ('Done', '#22d160')]:
        ...
        context_writer = models.ContextWriter(
            projects=[project],
        ...
        for i in range(20):
            ...
            for t in range(4):
                rand_task_status = random.choice(task_statuses_list)
                random_task_type = random.choice(
                    [tt for tt in task_types_list if tt.for_shots is True]
                )
                random_user = random.choice(users_list)
                shot.tasks.append(
                    models.Task(
        ...
        project_writer = models.ProjectWriter(
            project=project,
            shots=shots_list,
            assets=assets_list,
